...
def main(args):
    ...
    # 1) Load DTI matrix and labels of the atlas
    sCM    = load_mat(args.mat or DTI_MAT)
    labels = load_labels(args.labels or LABELS_TXT)

    # 2) Load the susceptibility-affected ROIS and delete it from the dti matrix
    ...
    # 3) Detecta comunidades en la SC limpia
    ...
    # red funcional dominante en cada módulo
    mod_networks = {}
    ...
    # 4) Calcula FC limpia para cada condición
    conds = {}
    ...
    # ===== APPROACH 1 =====
    # 1.1 sCM reordenada
    fig = plot_reordered_matrix(sCM_clean, Ci, title="sCM Reordenada")
    ...
    axs[0].set(title='Intramodular +', xlabel='Módulo')
    axs[0].legend(); axs[0].grid(True)
    axs[1].set(title='Intramodular –', xlabel='Módulo')
    axs[1].legend(); axs[1].grid(True)
    fig.tight_layout()
    fig.savefig(os.path.join(p1, "intramodular.png"))
    plt.close(fig)

    # 1.4 Intermodular anotado
    ...
    # --------------------------------------------------
    # 2.0 Carga etiquetas VTA y calcula índices tras limpieza
    ...
    # 2.1 Positive / negative mean per ROI
    ...
    # 2.2 Extraer series temporales y calcular entropías
    conds_ts = {}
    for cond in ('pre','post_on','post_off'):
        tpl       = str(PREP_TEMPLATES[cond])
    ...
    # 2.3 Generar PNG por ROI con las 3 condiciones
    ...
    # 2.6 Top 5 ROIs con mayor cambio global |ON−PRE|
    delta_global = []
    ...
    print("\nTop 5 ROIs con mayor cambio global |ON−PRE|:")
    for idx in top5:
    ...
    # imprime conectividad intrablock motor
