...
def compute_correlation_matrix(time_series, kind='correlation', standardize='zscore_sample'):
    ...
    Args:
        time_series: array de shape (n_timepoints, n_regions) o lista de estos arrays
        kind: 'correlation' | 'partial_correlation' | 'tangent'
        standardize: método de estandarización (solo para correlation)
...
def detect_communities(adj_matrix, seed=None, gamma=1.0, method='louvain'):
    ...
    Args:
        adj_matrix: np.ndarray simétrica (n x n)
        seed: semilla para reproducibilidad
        gamma: parámetro de resolución para modularidad
        method: 'louvain' (otro método podría implementarse)

    Returns:
        Tuple (ci, Q) donde:
          - ci: array de etiquetas comunitarias (length n)
          - Q: modularidad
    """
...
def mean_intermod_positive(cm, nodes):
    all_idx = np.arange(cm.shape[0])
    other = np.setdiff1d(all_idx, nodes)
    vals = cm[np.ix_(nodes, other)]
    pos = vals[vals > 0]
    return pos.mean() if pos.size else 0

def mean_intermod_negative(cm, nodes):
    all_idx = np.arange(cm.shape[0])
    other = np.setdiff1d(all_idx, nodes)
    vals = cm[np.ix_(nodes, other)]
    neg = vals[vals < 0]
